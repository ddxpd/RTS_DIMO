...
func run() -> void:
	...
	click(game.sim.buildings[1].pos)
	check(game.selected_building == 1 and game.selected_units.is_empty(), "Building selection")
	game._produce("harvester")
	check(game.sim.buildings[1].queue.size() == 1, "Production UI callback enqueues")
	game._begin_build("barracks")
	click(Vector2(448, 160))
	check(game.sim.buildings.size() == 3 and game.build_mode == "", "Build preview click creates construction")
