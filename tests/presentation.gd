...
func click(world: Vector2, button: int = MOUSE_BUTTON_LEFT) -> void:
    ...
    event.position     = game._world_to_screen(world)
...
func run() -> void:
    ...
    var event := InputEventMouseButton.new()
    event.position     = game._world_to_screen(Vector2(560, 240))
    event.button_index = MOUSE_BUTTON_LEFT
    event.pressed      = true
    root.push_input(event, true)
    event          = event.duplicate()
    event.position = game._world_to_screen(Vector2(680, 340))
    event.pressed  = false
    root.push_input(event, true)
    ...
    hud_press.position = game._world_to_screen(Vector2(560, 240))
    ...
    hud_release.position = game._world_to_screen(Vector2(700, 1150))
    ...
    cancel_press.position = game._world_to_screen(Vector2(560, 240))
    ...
    real_release.position = game._world_to_screen(Vector2(700, 1150))
    ...
    box_press.position = game._world_to_screen(Vector2(560, 240))
    root.push_input(box_press, true)
    var box_motion := InputEventMouseMotion.new()
    box_motion.position = game._world_to_screen(Vector2(680, 340))
    root.push_input(box_motion, true)
    game.selection_dragging = true
    game.left_button_held = true
    game.selection_start = Vector2(560, 240)
    game.selection_current = Vector2(680, 340)
    game.last_mouse_event_msec = Time.get_ticks_msec()
    ...
    check(game.selection_dragging and game.selection_start.distance_to(Vector2(560, 240)) < 0.1, "Duplicate press keeps the original box")
    ...
    stale_press.position = game._world_to_screen(Vector2(680, 340))
    root.push_input(stale_press, true)
    check(game.selection_dragging and game.selection_start.distance_to(Vector2(680, 340)) < 1.0, "Stale drag restarts from the new press")
    ...
    game._left_click(game.sim.buildings[refinery].pos)
    ...
    wheel.pressed      = true
    var zoom: float = game.camera_zoom_level
    root.push_input(wheel, true)
    check(game.camera_zoom_level > zoom, "Mouse wheel zoom")
    game.camera_zoom_level = 1.0
    game.camera_controller.focus = Vector2(500, 350)
    game._limit_camera()
