...
func run() -> void:
	...
	s.command(1, {"action": "produce", "building": 1, "type": "harvester"})
	s.command(1, {"action": "cancel_production", "building": 1})
	...
	advance(s, 3)
	check(not s.buildings.has(2) and s.winner == 1, "Destroy last enemy base to win")
	var frozen := s.frame
	s.step()
